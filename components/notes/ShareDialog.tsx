...
export function ShareDialog({ noteId, open, onClose }: { noteId: string; open: boolean; onClose: () => void }) {
  ...
  const load = React.useCallback(() => api<{ links: LinkVM[]; allowed: boolean }>(`/api/notes/${noteId}/share`).then((r) => { setLinks(r.links); setAllowed(r.allowed) }).catch((e) => toast.push({ text: String(e), tone: 'danger' })), [noteId, toast])
  ...
  return (
    <Dialog title={<span className="inline-flex items-center gap-2"><Globe className="h-4 w-4 text-accent" /> Share a read-only link</span>} onClose={onClose}>
      <p className="mb-3 text-[13px] text-fg-2">Anyone with the link can read this note (title and body only, no AI content, no tasks). You can revoke a link at any time.</p>
