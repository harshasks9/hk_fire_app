...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const b = (await req.json().catch(() => ({}))) as { status?: 'active' | 'disabled'; role?: 'owner' | 'member' }
    ...
    await logAdminEvent({ id: s.userId, name: s.user.name }, b.status ? (b.status === 'disabled' ? 'user.disable' : 'user.enable') : 'user.role', { type: 'user', id, name: u.name }, { role: b.role })
