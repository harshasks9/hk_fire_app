...
- **Any document becomes a note** — *Upload documents* (Notes and Inbox headers, `⌘K` → *Upload documents*, or Settings → Import) takes PDFs, Word (`.docx`), Excel (`.xlsx`), CSV/TSV, HTML, text and Markdown files, and scans or photos of documents (`.png`, `.jpg`, …), many at once, up to 40 MB each (uploaded in pieces). Each file becomes a note the moment the upload starts, the text is extracted in the background, the original stays attached under *Sources*, and the note then goes through the same pipeline as anything you write: people, companies, tasks, decisions, numbers, tags, embeddings and a summary. Notes → *Documents* lists them.
- **What gets extracted** (`lib/documents/extract.ts`): PDF text layers with the document's own title from its metadata (pages are marked, hyphenation and line wraps are repaired); Word headings, lists, tables and emphasis as Markdown; every sheet of a workbook and every CSV as a Markdown table (first 400 rows × 30 columns, the rest stays in the file); HTML with headings, lists, links and tables; text and Markdown with front matter. A PDF with no text layer (a scan) and any image are read by the model (Gemini) and marked as such at the top of the note; without a Gemini key the file is attached with a note saying so, and *Try again* on the upload dialog re-runs extraction once a key is added.
- **Progress and API** — the dialog shows *Uploading → Extracting text → AI is filing it → Done* per file and can be closed at any point; notes fill in on their own. `POST /api/documents` (multipart `files`, optional `contextId` or `context` slug; session cookie or a capture token) creates notes from files up to 4 MB in one request; larger files use `POST /api/documents/upload` (JSON `name`, `mime`, `size`), `PUT /api/documents/upload/<attachmentId>` per chunk (`x-chunk-last: 1` on the final piece) and `POST /api/documents/upload/<noteId>` to start extraction; `GET /api/documents/<noteId>` reports the stage and `POST` re-runs it. Quick capture uses the same extractor, so a PDF dropped into ⌘⇧N is searchable text, not just an attachment.

## Today

- **One page per day** — `/today` (sidebar, `⌘K` → *Today*, or the date on Home) shows the day in one place: the **journal**, the day's **schedule** (meetings, with *Prep* for the ones still ahead), tasks **due** that day (plus everything overdue, when the day is today), tasks **done** that day, notes **captured** that day, **decisions** made and **numbers that changed**, and, on today, the open loops that have gone stale. Arrows and the week strip (a dot marks days with captures) move between days; `?d=YYYY-MM-DD` deep-links a day.
...
- **A context or the whole notebook** — Settings → Data → *Export everything (.zip)* or *Export one context*: one folder per context with `<date> <title>.md` per note, `attachments/` beside them (linked relatively from the notes), `data.json` with the structured layer (people, companies, topics, tasks, decisions and their history, commitments, numbers, changes, meetings, research) and a README. `GET /api/export?context=<id>|all`.
- **Round-trips** — the Markdown the app writes is exactly what Settings → Import reads back: titles, dates, context and tags survive, and a line that is only an image becomes an image block again. Nothing in an export needs this app to be read.
...
- **Import** — Settings → Import takes Markdown/text files (front matter or first heading become the title, dates are preserved) and the app's own JSON export (see *Export*); duplicates are skipped and every note is analyzed. PDFs, Word, Excel, CSV, HTML and images go through *Upload documents* (see above).
