...
export function Shell({ children }: { children: React.ReactNode }) {
  const { state, role, setRole } = useProject();
        ...
        <div className="p-2.5 space-y-2">
          ...
          <div className="relative">
            <button onClick={() => setRoleOpen((v) => !v)} className="btn w-full justify-between">
              <span className="text-[12.5px]">{ROLES.find((r) => r.id === role)?.label}</span>
              <span className="text-ink-4 text-[10px]">switch</span>
            </button>
            {roleOpen && (
              <>
                <div className="fixed inset-0 z-20" onClick={() => setRoleOpen(false)} />
                <div className="absolute bottom-full mb-2 left-0 right-0 z-30 card p-1.5 shadow-xl animate-rise">
                  {ROLES.map((r) => (
                    <button
                      key={r.id}
                    ...
                    >
                      <div className="text-[13px] font-medium flex items-center gap-2">
                        {r.label}
                        {role === r.id && <span className="text-clay text-[11px]">current</span>}
                      </div>
      ...
      <header className="lg:hidden sticky top-0 z-30 bg-paper/92 backdrop-blur border-b border-line">
        <div className="flex items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setRole(role === "homeowner" ? "designer" : role === "designer" ? "vendor" : "homeowner")} className="btn btn-sm">
