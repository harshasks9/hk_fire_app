...
export function dotFor(kind: string): string {
  return kind === 'all' ? 'bg-[linear-gradient(135deg,var(--accent),var(--success),var(--warning))]' : kind === 'work' ? 'bg-accent' : kind === 'personal' ? 'bg-success' : kind === 'finance' ? 'bg-warning' : kind === 'family' ? 'bg-danger/80' : 'bg-fg-3'
}
